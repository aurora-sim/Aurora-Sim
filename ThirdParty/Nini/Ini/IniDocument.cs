	...
	public class IniDocument
	{
		...
		public void Save (TextWriter textWriter)
		{
			...
			for (int j = 0; j < sections.Count; j++)
			{
				...
				for (int i = 0; i < section.ItemCount; i++)
				{
					...
					{
					case IniType.Key:
						writer.WriteKey (item.Name, item.GetValue(), item.Comment);
						break;
					case IniType.Empty:
			...
			try {
				while (reader.Read ())
				{
					...
					case IniType.Section:
						...
						section = new IniSection (reader.Name, reader.Comment, reader.FixQuotes );
		...
		private IniReader GetIniReader (TextReader reader, IniFileType type)
		{
            ...
            case IniFileType.WindowsStyle:
                ...
                break;
            case IniFileType.AuroraStyle:
                result.FixQuotes = true;
                break;
			}
